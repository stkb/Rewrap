module internal rec Parsing.ReStructuredText
...
let rec reStructuredText (settings: Settings): TotalParser<string> =
    ...
    /// Ignores literal blocks (... ::)
    ///
    /// The beginning starts with any line with a :: suffix,
    /// followed by a blank line
    ///
    /// The ending is determined by the first non-blank line
    let literalBlock (Nonempty(headLine: string, tailLines: List<string>) as lines) =

        match tailLines with
            | [] -> None
            | x :: xs when Line.isBlank x = false -> None
            | _ :: xs ->
                let header, suffix = Line.split (Regex "::") headLine
                let hasStartMarker = String.length suffix > 0

                let takeLinesUntilStartNonEmpty : Lines * Option<Lines> =
                    let contentLines, otherLines =
                        List.span (lineStartsWith "\\s\\t") xs
                    ...
                    let outputLines =
                        Nonempty (header, contentLines @ maybeEndLine)
                    
                    (outputLines, maybeRemainingLines)
                
                if Line.endsWith "::" headLine then
                    takeLinesUntilStartNonEmpty
    ...
    let blockQuote =
        ...
        let mapper lines =
            ...
            Block.splitUp mkDefPrefix (reStructuredText settings) (prefixes, map snd splitLines)
        ...
        let doStuff listItemPrefix =
            ...
            ( ( (headPrefix, (String.replicate (String.length headPrefix) " "))
              , Nonempty
                    ...
                    |> Block.oldSplitUp (reStructuredText settings)
            , remainingLines
            )
...
/// reStructuredText parser in a line-by-line, state-based way like below. But will do
