// When we replace lines of text in the editor with new text that's been wrapped, the
// original cursor position/selections can get messed up; ie. the cursor isn't by the word
// it used to be.
//
// This module does a diff of the old and new text (using the js module fast-diff), and
// uses it to calculate where the selections should be after wrapping, so they can be
// re-applied in the editor after the edit(s) have been done.
import * as vscode from 'vscode'
const fd = require('fast-diff');

/** Given lines of original text, a set of selections and a set of edits, returns the
 *  positions of the selections for after the edits have been applied. */
...
/** Gets the new offset of a position, given and old offset and a diff between old and new
 *  text. */
function newOffsetFromOld (offset, diff) {
  // Count up chars from parts of the diff until we get to the original offset. Keep count
  // of the delta between old & new text from added & removed chars.
