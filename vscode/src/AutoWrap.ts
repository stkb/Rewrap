...
const checkChange = (e: TextDocumentChangeEvent) => {
    ...
    try {
        ...
        if (!edit.isEmpty) return editor.edit (builder => buildEdit(editor, builder, edit, false)).then (null, catchErr)
    }
...
export default function(memento: Memento) {
    ...
    editorListener = window.onDidChangeActiveTextEditor(e => e && onChangeEditor(e))
    configListener = workspace.onDidChangeConfiguration(e => ifActiveDoc(
        ed => e.affectsConfiguration('rewrap.autoWrap', ed.document) && onChangeEditor(ed)))
